...
impl From<OSDScalingArgs> for ScalingArgs {
	fn from(osd_scaling_args: OSDScalingArgs) -> Self {
		Self {
			target_resolution: None,
			scaling: osd_scaling_args.osd_scaling,
			no_scaling: osd_scaling_args.no_osd_scaling,
			min_margins: osd_scaling_args.min_osd_margins,
			min_coverage: osd_scaling_args.min_osd_coverage,
		}
	}
}
	...
	pub fn try_from_osd_args(
		...
		Ok(match (args.osd_scaling, args.no_osd_scaling) {
			...
			(false, true) => Scaling::No {
				target_resolution: Some(TargetResolution::Custom(video_resolution)),
			},
			(false, false) => {
