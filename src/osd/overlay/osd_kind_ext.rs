...
impl osd::Kind {
	...
	pub fn best_kind_of_tiles_to_use_without_scaling(
	...
	) -> Result<tile::Kind, VideoResolutionTooSmallError> {
		let avg_margins = tile::Kind::iter().filter_map(|tile_kind| {
		...
		match avg_margins.min_by_key(|(_, margin_avg)| *margin_avg) {
			...
			None => Err(VideoResolutionTooSmallError {
				osd_kind: *self,
				video_resolution,
			}),
		}
	...
	#[must_use]
	pub fn best_kind_of_tiles_to_use_with_scaling(
		&self,
		max_resolution: OverlayFrameDimensions,
	) -> (tile::Kind, tile::Dimensions, OverlayFrameDimensions) {
		...
		let tile_kinds_data = tile::Kind::iter()
			.map(|tile_kind| {
				...
				(
					...
					width_diff,
					height_diff,
					std::cmp::min(width_diff.abs(), height_diff.abs()),
				)
			})
			.collect::<Vec<_>>();

		// look for kinds for which we would downscale tiles
		let downscaling_tile_kinds_data = tile_kinds_data
			.iter()
			.filter(|(_, width_diff, height_diff, _)| std::cmp::min(*width_diff, *height_diff) <= 0)
			.collect::<Vec<_>>();
		...
		if width_diff < height_diff {
			#[allow(clippy::cast_possible_wrap)]
			let tile_width = tile_dimensions.width as i32;
			tile_dimensions.width = (tile_width + width_diff).try_into().unwrap();
			tile_dimensions.height = tile_dimensions.height * tile_dimensions.width / tile_kind.dimensions().width;
		} else {
			#[allow(clippy::cast_possible_wrap)]
			let tile_height = tile_dimensions.height as i32;
			tile_dimensions.height = (tile_height + height_diff).try_into().unwrap();
