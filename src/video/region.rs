...
impl Region {
	...
	#[must_use]
	pub fn new4(
		x: super::SignedCoordinate,
		y: super::SignedCoordinate,
		width: super::Dimension,
		height: super::Dimension,
	) -> Self {
		Self {
	...
	pub fn bottom_right_corner(&self) -> super::SignedCoordinates {
		...
		let x = self.top_left_corner.x() + self.dimensions.width as super::SignedCoordinate - 1;
		#[allow(clippy::cast_possible_wrap)]
		let y = self.top_left_corner.y() + self.dimensions.height as super::SignedCoordinate - 1;
		super::SignedCoordinates { x, y }
	}
...
impl FromStr for Region {
	...
	fn from_str(s: &str) -> Result<Self, Self::Err> {
		Ok(if let Some((origin_s, dimensions_s)) = s.split_once(':') {
			let origin = super::Coordinates::from_str(origin_s).map_err(|error| FormatError::Origin {
				value: origin_s.to_owned(),
				error,
			})?;
			let dimensions = super::Dimensions::from_str(dimensions_s).map_err(|error| FormatError::Dimensions {
			...
			let origin = super::Coordinates::from_str(s).map_err(|error| FormatError::Origin {
				value: s.to_owned(),
				error,
			})?;
			Region {
