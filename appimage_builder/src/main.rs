...
const DEP_BINARIES: [&str; 2] = ["ffmpeg", "mpv"];
...
fn create_path<P: AsRef<Path>>(path: P) -> anyhow::Result<()> {
	std::fs::create_dir_all(&path)
		.map_err(|error| anyhow!("failed to create dir `{}`: {error}", path.as_ref().to_string_lossy()))
}
...
fn copy_file<P: AsRef<Path>, Q: AsRef<Path>>(from_path: P, to_path: Q, permissions_mode: u32) -> anyhow::Result<()> {
	...
	std::fs::copy(&from_path, to_path).with_context(|| {
		format!(
			"failed to copy `{}` => `{}`",
			from_path.as_ref().to_string_lossy(),
...
fn binary_linked_libs<P: AsRef<Path>>(bin_path: P) -> anyhow::Result<Vec<PathBuf>> {
	...
	Ok(lib_re
		.captures_iter(ldd_output)
		.map(|captures| PathBuf::from(captures.get(1).unwrap().as_str().to_string()))
		.collect())
}
...
fn install_binary_shared_libs<P: AsRef<Path>, Q: AsRef<Path>>(binary_path: P, lib_dir_path: Q) -> anyhow::Result<()> {
	...
	for lib_path in binary_linked_libs(&binary_path)? {
		...
		if EXCLUDE_LIBS
			.iter()
			.any(|ex_name| lib_file_name.starts_with(&format!("{ex_name}.")))
		{
			continue;
		}
		...
		copy_file(&lib_path, &to_path, 0o644).with_context(|| {
			format!(
				"{} linked libs copy: failed copying `{}` => `{}`",
...
fn install_binary_dependency<P: AsRef<Path>, Q: AsRef<Path>, R: AsRef<Path>>(
	binary_path: P,
	bin_dir_path: Q,
	lib_dir_path: R,
) -> anyhow::Result<()> {
	let binary_path_str = binary_path.as_ref().to_string_lossy();
	if !binary_path.as_ref().exists() {
		return Err(anyhow!("binary dependency not found: {binary_path_str}"));
	}
	...
	copy_file(&binary_path, &bin_dest_path, 0o755).with_context(|| {
		format!(
			"failed to install binary dependency at {}",
			bin_dest_path.to_string_lossy()
		)
	})?;
...
fn install_application_binary<P: AsRef<Path>, Q: AsRef<Path>>(binary_path: P, bin_dir_path: Q) -> anyhow::Result<()> {
	...
	std::fs::copy(binary_path, &binary_dest_path).with_context(|| {
		format!(
			"failed to install application binary at {}",
			binary_dest_path.to_string_lossy()
		)
	})?;
	Ok(())
}

fn install_desktop_file<P: AsRef<Path>>(
	appdir_path: P,
...
async fn prepare_appimagetool() -> anyhow::Result<PathBuf> {
	...
	if !appimagetool_path.exists() {
		...
		download_file_with_progress(APPIMAGETOOL_URL, APPIMAGETOOL_BIN_NAME)
			.await
			.context("appimagetool")?;
	}

	if !appimagetool_path.is_file() {
		log::error!("{APPIMAGETOOL_BIN_NAME} exists but is not a regular file");
	}

	std::fs::set_permissions(appimagetool_path, fs::Permissions::from_mode(0o755))
		.context(format!("failed to set {APPIMAGETOOL_BIN_NAME} permissions"))?;

	Ok([Path::new("."), appimagetool_path].iter().collect())
}

fn generate_appimage<P: AsRef<Path>, Q: AsRef<Path>, R: AsRef<Path>>(
	appimagetool_bin_path: P,
	appimage_path: Q,
	appdir_path: R,
) -> anyhow::Result<()> {
	...
	if !appimagetool_output.status.success() {
		log::error!(
			"failed to generate AppImage image: {APPIMAGETOOL_BIN_NAME}: {}",
			appimagetool_output.status
		);
		println!();
